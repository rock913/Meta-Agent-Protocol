...
```bash
# 1. 在你的仓库中添加 MAP 作为 submodule
git submodule add https://github.com/rock913/Meta-Agent-Protocol.git conventions

# 2. 运行脚手架脚本
bash conventions/scripts/setup_map.sh

# 3. 编辑生成文件中的 {{PLACEHOLDER}}，填入你的项目信息
#   - AI_RULES.md: 子项目名、PRD 路径、测试命令
#   - .aider.conf.yml: 工作空间名称、PRD/ROADMAP 路径映射
#   - .importlinter: Python 包名列表

# 4. 提交
git add -A && git commit -m "feat: bootstrap MAP v1.0.1"
```

> 📖 **详细步骤**（含每个占位符的含义、New/Migration 两种路径、Makefile 模板、验证清单）→ **[INTEGRATION.md](./INTEGRATION.md)**
...
Meta-Agent-Protocol/
├── README.md                       # 本文件
├── INTEGRATION.md                  # 宿主仓库接入指引（升级/全新安装）
